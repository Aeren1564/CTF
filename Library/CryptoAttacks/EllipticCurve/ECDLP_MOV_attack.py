...
def ECDLP_MOV_attack(EC, P, Q):
	...
	for d in range(1, 21):
		...
		print(f"[ERROR] <ECDLP_MOV_attack> Embedding degree is greater than 20")
		assert False
	print(f"[INFO] <ECDLP_MOV_attack> Embedding degree is {d} and the extension field is of order {p**(k * d)}")
	...
	while True:
		...
		if R.order() == P.order() and R.weil_pairing(P, P.order()) != 1:
			break
	print(f"[INFO] <ECDLP_MOV_attack> {R = }")
