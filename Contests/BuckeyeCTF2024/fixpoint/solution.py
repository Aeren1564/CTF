...
for i in range(250):
	s = fixed_point[3 * i : 3 * i + 3]
	s = base64.b64encode(s.encode()).decode()
	t = fixed_point[4 * i : 4 * i + 4]
	for j in range(4):
		if s[j] in standard_alphabet:
			ind = standard_alphabet.index(s[j])
			alphabet = alphabet[: ind] + t[j] + alphabet[ind + 1 :]
...
print(encode(fixed_point))
assert fixed_point == encode(fixed_point)[: 1000]
print(f"{alphabet = }")