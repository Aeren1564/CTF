...
atomic<int> found_env_seed = -1;
vector<pair<unsigned int, array<int, 52>>> outputs;
void check(unsigned int rem, unsigned int stride){
	for(auto env_seed = rem; env_seed < 0xffffff && !~found_env_seed; env_seed += stride){
		for(auto &[seed, output]: outputs){
			if(shuffle(env_seed, seed) != output){
				goto FAIL;
			}
		}
		cout << "Found!, " << env_seed << endl;
		found_env_seed = env_seed;
		break;
		FAIL:;
	}
}
...
int main(){
	ifstream fin("remote_output.txt");
	while(true){
		string cur;
		if(!getline(fin, cur)){
			break;
	...
	for(auto rem = 0; rem < num_threads; ++ rem){
		threads.emplace_back(check, rem, num_threads);
	}
	for(auto &t: threads){
		t.join();
	}
	if(!~found_env_seed){
		cout << "Failed to find correct env seed\n";
		assert(false);
	}
	ofstream fout("env_seed.txt");
	fout << found_env_seed << "\n";
	return 0;
}
